...
class ModuleSummaryIndexBitcodeReader : public BitcodeReaderBase {
  ...
  std::vector<std::string> PendingCfiFunctionDefs;
  std::vector<std::string> PendingCfiFunctionDecls;
...
private:
  ...
  unsigned getStackIdIndex(unsigned LocalIndex) const;
};
...
// Parse the summary prefix serially. This interns all ValueInfos and stack ids
// before any readers operate concurrently on the combined index.
...
Error ModuleSummaryIndexBitcodeReader::parseSummaryBody() {
  ...
  while (true) {
    ...
    case bitc::FS_PERMODULE_PROFILE: {
      ...
      addSummary(std::get<0>(VIAndOriginalGUID), std::move(FS));
      break;
    }
    ...
    case bitc::FS_CFI_FUNCTION_DEFS: {
      for (unsigned I = 0; I != Record.size(); I += 2) {
        ...
        if (DeferMerge)
          PendingCfiFunctionDefs.emplace_back(Name);
        else
          TheIndex.cfiFunctionDefs().emplace(Name);
      }
      break;
    }
    ...
    case bitc::FS_CFI_FUNCTION_DECLS: {
      for (unsigned I = 0; I != Record.size(); I += 2) {
        StringRef Name(Strtab.data() + Record[I],
                       static_cast<size_t>(Record[I + 1]));
        if (DeferMerge)
          PendingCfiFunctionDecls.emplace_back(Name);
        else
          TheIndex.cfiFunctionDecls().emplace(Name);
      }
      break;
    }
...
unsigned
ModuleSummaryIndexBitcodeReader::getStackIdIndex(unsigned LocalIndex) const {
  assert(LocalIndex < StackIds.size());
  if (DeferMerge) {
    assert(LocalIndex < StackIdIndices.size());
    return StackIdIndices[LocalIndex];
  }
  return TheIndex.addOrGetStackIdIndex(StackIds[LocalIndex]);
}
...
void ModuleSummaryIndexBitcodeReader::mergePreparedSummary() {
  ...
  for (const std::string &Name : PendingCfiFunctionDefs)
    TheIndex.cfiFunctionDefs().emplace(Name);
  PendingCfiFunctionDefs.clear();

  for (const std::string &Name : PendingCfiFunctionDecls)
    TheIndex.cfiFunctionDecls().emplace(Name);
...
Error BitcodeModule::readSummary(
  ...
  ModuleSummaryIndexBitcodeReader R(std::move(Stream), Strtab, CombinedIndex,
                                    ModulePath, std::move(IsPrevailing));
...
BitcodeModule::prepareSummary(
  ...
  auto R = std::make_unique<ModuleSummaryIndexBitcodeReader>(
      ...
      std::move(IsPrevailing));
  if (Error Err = R->prepareModule())
    return std::move(Err);
