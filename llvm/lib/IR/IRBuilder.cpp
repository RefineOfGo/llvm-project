...
CallInst *IRBuilderBase::CreateMemSet(Intrinsic::ID IntrID, Value *Ptr,
                                      Value *Val, Value *Size, MaybeAlign Align,
                                      bool isVolatile,
  ...
  CallInst *CI = CreateIntrinsicWithoutFolding(IntrID, Tys, Ops);
...
CallInst *IRBuilderBase::CreateGCWrite(Value *Val, Value *Obj, Value *Ptr,
  ...
  CallInst *CI = CreateIntrinsicWithoutFolding(Intrinsic::gcwrite, Ops);
...
CallInst *IRBuilderBase::CreateMemTransferInst(Intrinsic::ID IntrID, Value *Dst,
  ...
  CallInst *MCI = CreateIntrinsicWithoutFolding(IntrID, Tys, Ops);

  if (DstAlign)
    MCI->addParamAttr(
        0, Attribute::getWithAlignment(MCI->getContext(), *DstAlign));
  if (SrcAlign)
    MCI->addParamAttr(
        1, Attribute::getWithAlignment(MCI->getContext(), *SrcAlign));
