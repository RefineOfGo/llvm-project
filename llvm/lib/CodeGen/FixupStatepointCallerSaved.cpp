...
// ROG slot-liveness stack maps. After register assignment (LiveIntervals and
// LiveStacks still valid, vregs not yet rewritten) record, for EVERY
// statepoint, all spill slots whose live interval covers it. Slot liveness is
// ground truth maintained by the register allocator itself (spill stores and
// reloads are explicit machine instructions), which sidesteps the whole
// value-tracking design this pass replaced and each of its measured failure
// modes: coalescing renames losing the captured handle (stale),
// rematerialization making roots "dead at the safepoint" while the program
// still needs them, dropped debug-value markers, and backend-materialized
// derived cursors. The recorded set is a conservative superset -- non-pointer
// spill slots are scanned too and filtered by the runtime's heap check --
// while dead slots stay excluded (no false retention).
//
// Completeness: a root live across a safepoint is either in a callee-saved
// register, covered by the runtime's conservative CSR-area scan, or in a
// stack slot, covered here; allocas (stack objects, invisible to LiveStacks)
// carry their Direct entries straight from the emitter's deopt bundle (plain
// FI + size operands, no register pressure, no stripping needed).
//
// Emitted records are the standard indirect quads [IndirectMemRefOp, size,
// FI, 0]; PEI resolves the frame index and StackSlotColoring (which runs
// later) remaps merged slots in these operands like any other. The runtime
// is unchanged.
//
// Measured on the value-tracking killers under GOGC=1 (full CI flag set):
// smx509 TestPathBuilding 0/18 failures, reedsolomon TestEncoding 0/3, with
// regexp machine.add frames at the statepoint-free baseline (0x78) and ~+2%
// stackmap section size.
class RogQueryDeopt : public MachineFunctionPass {
public:
  static char ID;
  RogQueryDeopt() : MachineFunctionPass(ID) {}
  StringRef getPassName() const override {
    return "ROG slot-liveness stack maps";
  }
  void getAnalysisUsage(AnalysisUsage &AU) const override {
  ...
  bool runOnMachineFunction(MachineFunction &MF) override {
    ...
    LiveStacks &LSS = getAnalysis<LiveStacksWrapperLegacy>().getLS();
    ...
    for (MachineBasicBlock &BB : MF)
      for (MachineInstr &MI : BB) {
        ...
        for (auto &Ent : LSS)
          if (!MFI.isDeadObjectIndex(Ent.first) && Ent.second.liveAt(S))
            LiveFIs.push_back(Ent.first);
        if (LiveFIs.empty())
          continue;
        llvm::sort(LiveFIs); // determinism: LiveStacks iterates a hash map
        unsigned VI = StatepointOpers(&MI).getVarIdx();
        if (VI + 5 >= MI.getNumOperands() || !MI.getOperand(VI + 5).isImm())
          continue;
        unsigned Prev = MI.getOperand(VI + 5).getImm();
